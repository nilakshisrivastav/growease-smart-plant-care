...
import 'package:grow_ease/Screen/cart_icon_with_badge.dart'; // Import the reusable widget
...
class SummerSeason extends StatefulWidget {
  const SummerSeason({super.key});

  @override
  State<SummerSeason> createState() => _SummerSeasonState();
}

int screenIndex = 0; // Define screen index for SummerSeason list

class _SummerSeasonState extends State<SummerSeason> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Summer Season"),
        actions: [
      ...
      body: ListView.builder(
        itemCount: SummerSeason_list.length,
        itemBuilder: (context, index) {
          // Check if the item is in the wishlist
          bool isWishlisted = wishlistNotifier.isItemInWishlist(SummerSeason_list[index]);
              ...
              child: InkWell(
                onTap: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => PlantDetailPage(plant: SummerSeason_list[index]),
                    ),
                  );
                },
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            SummerSeason_list[index].p_name,
                            style: const TextStyle(fontWeight: FontWeight.w800),
                          ),
                          Text(
                            "Rs.${SummerSeason_list[index].p_price}",
                            style: const TextStyle(fontWeight: FontWeight.w800),
                          ),
                          Text(
                            SummerSeason_list[index].p_benefits,
                            style: const TextStyle(fontWeight: FontWeight.w800),
                          ),
                          Row(
                            children: [
                              ...
                              InkWell(
                                onTap: () {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        "${SummerSeason_list[index].p_name} added to cart!",
                              ...
                              IconButton(
                                ...
                                onPressed: () {
                                  if (isWishlisted) {
                                    wishlistNotifier.removeFromWishlist(SummerSeason_list[index]);
                                  } else {
                                    wishlistNotifier.addToWishlist(SummerSeason_list[index]);
                                  }
                                },
                              ),
                            ],
